...
export class QuizComponent {
  ...
  public isSubmitButtonVisible(): boolean {
    return !this.isSubmitted && this._customQuestions?.length > 0 && this._customQuestions?.every((c: CustomQuestion) => c.selected_answers.length > 0);
  }

  public submitQuiz(): void {
    if(this.isSubmitButtonVisible()) {
      this.isSubmitted = true;
      this.quizSubmitted.emit(this._customQuestions);
    }
  }
}
